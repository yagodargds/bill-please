...
public class Test {
    ...
    private static void intMapTest() {
        ...
        intMap.put(2, "dva");
        intMap.put(3, "tri");
        intMap.put(4, "chetire");
        intMap.put(5, "pyat");

        for (String str : intMap.values()) {
            System.out.println(str + " of " + intMap.size);
            if(str.equals("tri")) {
                intMap.remove(3);
            }
        }
    ...
    private static void nullTest() {
        ...
        Array<Float> satTripProb = new Array<>();
        ...
        System.out.println("---------------------------------------------------------");
        calcUniformMonthNx(startDate, countM, endDate, getMonthN1Price() / 30.0f, satTripProb, new T_K1_UniformMonthTramRouteRule());
        System.out.println("---------------------------------------------------------");
        ...
        System.out.println("---------------------------------------------------------");
        calcUniformMonthNx(startDate, countM, endDate, getMonthN2Price() / 30.0f, satTripProb,  new T_B_UniformMonthTramBusRouteRule());
        System.out.println("---------------------------------------------------------");
        calcComboMonth(startDate, countM, endDate, satTripProb, new T_B_ComboMonthRouteRule());
    }

    private static void calcUniform(Calendar startDate, int countM, int countN, Calendar endDate, Array<Float> satTripProb, AbsUniformRouteRule routeRule) {
        ...
        while(curDate.before(endDate)) {
            if(curDate.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY) {
                satTrip = Random.raundV(satTripProb) == 1;
            } else {
                satTrip = false;
            }
    ...
    private static void calcUniformMonthNx(Calendar startDate, int countM, Calendar endDate, float dayPrice, Array<Float> satTripProb, AbsCountMRouteRule routeRule) {
        Calendar curDate = (Calendar) startDate.clone();
        boolean satTrip;
        CountMTripInfo tripInfo;
        float price = 0.0f;
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd.MM.yyyy E");
        System.out.println("calcUniformMonthNx {start_date={" + dateFormat.format(startDate.getTime()) + "} countM=" + countM + " price=" + price + "}");
        while(curDate.before(endDate)) {
            price += dayPrice;
    ...
    private static void calcComboMonth(Calendar startDate, int countM, Calendar endDate, Array<Float> satTripProb, AbsCountMRouteRule routeRule) {
        Calendar curDate = (Calendar) startDate.clone();
        boolean satTrip;
        CountMTripInfo tripInfo;
        ...
        public UniformTripInfo getTripInfoFrom(Calendar date, int countM, int countN, boolean satTrip) {
            ...
            if(dayOfWeek != Calendar.SATURDAY && dayOfWeek != Calendar.SUNDAY) {
                if(dayOfWeek == Calendar.MONDAY || dayOfWeek == Calendar.WEDNESDAY || dayOfWeek == Calendar.FRIDAY) {
                    tripInfo.countM = 2;
                    tripInfo.countN = 1;
                    tripInfo.price = getPriceK_1() + getUniformPriceM(countM) + getUniformPriceM(countM + 1) + getUnifromPriceN(countN);
                } else {
                    tripInfo.countM = 1;
                    tripInfo.countN = 1;
        ...
        public UniformTripInfo getTripInfoFrom(Calendar date, int countM, int countN, boolean satTrip) {
            ...
            if(dayOfWeek != Calendar.SATURDAY && dayOfWeek != Calendar.SUNDAY) {
                if(dayOfWeek == Calendar.MONDAY || dayOfWeek == Calendar.WEDNESDAY || dayOfWeek == Calendar.FRIDAY) {
                    tripInfo.countM = 2;
                    tripInfo.countN = 2;
                    tripInfo.price = getUnifromPriceN(countN) + getUniformPriceM(countM) + getUniformPriceM(countM + 1) + getUnifromPriceN(countN);
                } else {
                    tripInfo.countM = 1;
                    tripInfo.countN = 2;
                    tripInfo.price = getUnifromPriceN(countN) + getUniformPriceM(countM) + getUnifromPriceN(countN + 1);
    ...
    private static class T_K1_UniformMonthTramRouteRule extends AbsCountMRouteRule {
        @Override
        ...
        public CountMTripInfo getTripInfoFrom(Calendar date, int countM, boolean satTrip) {
            ...
            if(dayOfWeek != Calendar.SATURDAY && dayOfWeek != Calendar.SUNDAY) {
                if(dayOfWeek == Calendar.MONDAY || dayOfWeek == Calendar.WEDNESDAY || dayOfWeek == Calendar.FRIDAY) {
                    tripInfo.countM = 2;
                    tripInfo.price = getPriceK_1() + getUniformPriceM(countM) + getUniformPriceM(countM + 1) + 0.0f;
                } else {
                    tripInfo.countM = 1;
    ...
    private static class T_B_UniformMonthTramBusRouteRule extends AbsCountMRouteRule {
        @Override
        public CountMTripInfo getTripInfoTo(Calendar date, int countM, boolean satTrip) {
            CountMTripInfo tripInfo = new CountMTripInfo();
            int dayOfWeek = date.get(Calendar.DAY_OF_WEEK);
        ...
        public CountMTripInfo getTripInfoTo(Calendar date, int countM, boolean satTrip) {
            ...
            if(dayOfWeek != Calendar.SATURDAY && dayOfWeek != Calendar.SUNDAY) {
                ...
                if(countM <= 70) {
                    ...
                    tripInfo.price = 0.0f + getDefPriceM() + getPriceK_1();
                }
            } else if(dayOfWeek == Calendar.SATURDAY && satTrip) {
                tripInfo.countM = 1;
                if(countM <= 70) {
                    ...
                    tripInfo.price = 0.0f + getDefPriceM();
                }
            } else {
                tripInfo.countM = 0;
                tripInfo.price = 0.0f;
        ...
        public CountMTripInfo getTripInfoFrom(Calendar date, int countM, boolean satTrip) {
            ...
            if(dayOfWeek != Calendar.SATURDAY && dayOfWeek != Calendar.SUNDAY) {
                if(dayOfWeek == Calendar.MONDAY || dayOfWeek == Calendar.WEDNESDAY || dayOfWeek == Calendar.FRIDAY) {
                    tripInfo.countM = 2;
                    if(countM <= 70) {
                        tripInfo.price = getPriceK_1();
                        countM++;
                        if(countM <= 70) {
                            tripInfo.price += 0.0f;
                        } else {
                            tripInfo.price += getDefPriceM();
                        }
                        tripInfo.price += 0.0f;
                    } else {
                        tripInfo.price = getPriceK_1() + getDefPriceM() + getDefPriceM() + 0.0f;
                    }
                } else {
                    tripInfo.countM = 1;
                    if(countM <= 70) {
                        ...
                        tripInfo.price = getPriceK_1() + getDefPriceM() + 0.0f;
                    }
                }
            } else if(dayOfWeek == Calendar.SATURDAY && satTrip) {
                tripInfo.countM = 1;
        ...
        public CountMTripInfo getTripInfoTo(Calendar date, int countM, boolean satTrip) {
            ...
            if(dayOfWeek != Calendar.SATURDAY && dayOfWeek != Calendar.SUNDAY) {
                ...
                if(countM <= 70) {
                    ...
                    tripInfo.price = 0.0f + getDefPriceM() + 0.0f;
                }
            } else if(dayOfWeek == Calendar.SATURDAY && satTrip) {
                tripInfo.countM = 1;
                if(countM <= 70) {
            ...
            if(dayOfWeek != Calendar.SATURDAY && dayOfWeek != Calendar.SUNDAY) {
                if(dayOfWeek == Calendar.MONDAY || dayOfWeek == Calendar.WEDNESDAY || dayOfWeek == Calendar.FRIDAY) {
                    ...
                    if(countM <= 70) {
                        ...
                        tripInfo.price = 0.0f + getDefPriceM() + getDefPriceM() + 0.0f;
                    }
                } else {
                    tripInfo.countM = 1;
                    if(countM <= 70) {
    ...
    private static class CountMTripInfo {
        ...
        public String toString() {
            ...
            Utils.Debug.buildShortClassTag(this, sb);
